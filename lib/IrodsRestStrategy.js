...
class IrodsRestStrategy {
    ...
    storeFile(xtensDataFile, idData, dataTypeName, callback) {
        ...
        this.protocol.request(postOptions, cb);
     ...
     * @description delete a file from the iRODS file system through the irods-rest API
     * @param{string} fileUri - the URI of the file on iRODS ("virtual path")
     * @param{response} remoteRes - the "remote" Node.js response object
     * @param{function} callback - the callback function
     */
    deleteFileContent(fileUri, callback) {
        fileUri=fileUri.replace(/\s/g,"+");
        let deleteOptions = {
            hostname: this.restURL.hostname,
            port: this.restURL.port,
            path: this.restURL.path + '/collection' + fileUri,
            method: 'DELETE',
            auth: this.username + ':' + this.password
        };


        let deleteRequest = this.protocol.request(deleteOptions, function(res) {
            if (res.statusCode != 204) {
              console.log('irods.deleteFileContent - problem while deleteing file: ' + err.message);
              callback(err);
            }

            console.log("irods.deleteFileContent - file correctly deleted");
            callback();
        });
