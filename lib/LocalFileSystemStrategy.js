...
class LocalFileSystemStrategy {
    ...
    storeFile(xtensDataFile, idData, dataTypeName, callback) {
        ...
        // if an URI property is provided for file, it is not moved
        if (xtensDataFile.uri) {
            if (fs.statSync(xtensDataFile.uri).isFile()) {
                callback();
                return;
            }
            else {
                throw new Error("LocalFileSystemStrategy.storeFile - path does not correspond to existing file");
            }
        }

        let _this = this;
        let fileName = xtensDataFile.name.split("/")[xtensDataFile.name.split("/").length - 1];
        let source = path.isAbsolute(xtensDataFile.name) ? xtensDataFile.name : this.path + "/" + this.landingDirectory + "/" + xtensDataFile.name;
        // var destination = this.path + "/" + this.repoDirectory + "/" + dataTypeName + "/" + idData + "/" + fileName;
        let destination = path.resolve(this.path, this.repoDirectory, dataTypeName, idData.toString(), fileName);
        console.log("LocalFileSystemStrategy.storeFile - destination: " + destination);
     ...
     */
    deleteFileContent(uri, callback) {
       // TODO: test this!!
      fs.unlink(uri , (err) => {
        if (err) {
          callback(err);
